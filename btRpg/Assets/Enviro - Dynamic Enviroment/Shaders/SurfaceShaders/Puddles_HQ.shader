    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
				...
				half linear01Depth = Linear01Depth(i.projPos.z);
				float4 fog = TransparentFog(float4(diffuse,0), i.posWorld, i.projPos.xy * 0.5 + 0.5, linear01Depth);
                float3 finalColor = fog.rgb + specular;
                fixed4 finalRGBA = fixed4(finalColor,(saturate((sceneZ-partZ)/_Depth)*_Alpha));
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);

                return float4(finalRGBA);
            }
            ENDCG
        }
        Pass {
