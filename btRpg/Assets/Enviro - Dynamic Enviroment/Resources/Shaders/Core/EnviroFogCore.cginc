...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
 half ComputeFogFactor (float coord)
	...
	float4 ComputeScattering (float3 viewDir, float2 sunPos)
	{
		   ...
		   scattering *= saturate((lerp(_scatteringPower , pow(2000.0 * BrmTheta * fex,0.7),sunPos.y) * 0.05));
		   scattering *= (_SkyLuminance * _scatteringColor.rgb * _scatteringStrenght) * pow((1.0 - fex),1.0) * sunPos.x;

		   float3 skyFinalize = saturate((pow( 1.0 - fex, 2.0) * 0.234) * (1 - sunPos.x)); 
	...
	float4 fogClr = ComputeScattering(normalize(wsDir),sunDir);
