Shader "Enviro/RaymarchClouds"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4x4 _InverseProjection_SP;
			float4x4 _InverseRotation_SP;
			...
			float _HgPhaseFactor;
			float _BaseNoiseUV;
			float _DetailNoiseUV;
			...
			float4 _CloudsAnimation;
			float _Tonemapping;
			float _CloudsExposure;
			float3 _SunDir1;
			float3 _MoonDir;
			float _gameTime;
			float _PrimAtt;
			...
			float CalculateCloudDensity(float3 pos, float height, float3 weather, bool details)
			{
			    ...
			    if(details)
			    {
					...
					float high_freq_noise_modifier = lerp(high_freq_fBm, 1.0 - high_freq_fBm, saturate(height * 10.0));
		 			cloudDensity = Remap(cloudDensity, high_freq_noise_modifier * 0.4, 1.0, 0.0, 1.0);
				}
		        ///
			...
			{						
				...
				int steps = (int)lerp(_Steps.x, _Steps.x * 0.5, abs(ray.y));
