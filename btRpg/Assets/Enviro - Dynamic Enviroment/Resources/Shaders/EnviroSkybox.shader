...
Shader "Enviro/Skybox"
{
	Properties
	{
		...
		_noiseIntensity ("Noise Intensity", Float ) = 4
		_CloudAnimationSpeed ("Animation Speed", Float ) = 0.1
	}
	SubShader 
	{
    	...
    	Pass 
    	{
			...
			float4 frag(v2f i) : SV_Target
			{
               ...
               float3 skyScattering = (scattering + sunDisk) + (skyFinalize + stars + ((moonBright * _moonGlowColor)*_moonGlowStrenght));
